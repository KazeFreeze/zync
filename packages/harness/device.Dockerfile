...
# and tsx are all resolvable; the daemon is run via tsx (NO compile step).
...
# The headless-client needs core + crdt-yjs + server(devDep) + tsx. Installing
# that filtered closure keeps the image lean while pulling everything tsx needs
# to run the daemon end-to-end.
