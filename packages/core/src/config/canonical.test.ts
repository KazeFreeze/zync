...
describe("canonicalJsonBytes", () => {
  it("sorts keys so reordering is identical", () => {
    expect(canonicalJsonBytes(enc(`{"b":1,"a":2}`))).toEqual(canonicalJsonBytes(enc(`{"a":2,"b":1}`)));
  });
  it("normalizes whitespace", () => {
    expect(canonicalJsonBytes(enc(`{ "a":  1 }`))).toEqual(canonicalJsonBytes(enc(`{"a":1}`)));
  });
  it("sorts nested keys", () => {
    expect(canonicalJsonBytes(enc(`{"o":{"y":1,"x":2}}`))).toEqual(canonicalJsonBytes(enc(`{"o":{"x":2,"y":1}}`)));
  });
...
describe("configIdentitySha", () => {
  it("plugin-data: key-reorder yields the same sha", async () => {
    expect(await configIdentitySha(dataPath, enc(`{"b":1,"a":2}`)))
      .toBe(await configIdentitySha(dataPath, enc(`{"a":2,"b":1}`)));
  });
  it("plugin-data: value change yields a different sha", async () => {
    expect(await configIdentitySha(dataPath, enc(`{"a":1}`)))
      .not.toBe(await configIdentitySha(dataPath, enc(`{"a":2}`)));
  });
